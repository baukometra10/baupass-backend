...
class ContractsService:
    ...
    def send_sign_invite_email(
        ...
        *,
        ...
        subject, text_body, html_body = self._sign_invite_email_bodies(
            lang=lang,
            role=role,
            company_name=str(self._load_company(company_id).get("name") or "BauPass"),
    ...
    def _sign_invite_sms_body(*, lang: str, contract_title: str, sign_url: str) -> str:
        lang = normalize_lang(lang)
        if lang == "en":
            return f"BauPass: Please sign \"{contract_title}\": {sign_url}"
        if lang == "ar":
            return f"BauPass: يرجى توقيع «{contract_title}»: {sign_url}"
        return f"BauPass: Bitte unterschreiben Sie „{contract_title}“: {sign_url}"
    ...
    def send_sign_session_reminder(self, session: dict[str, Any], *, base_url: str = "") -> bool:
        ...
        company = self._load_company(company_id)
        company_name = str(company.get("portal_display_name") or company.get("name") or "BauPass")
        recipient = ""
        if role == "employee":
    ...
    def _notify_counterparty_signed(self, contract: dict[str, Any] | None, *, role: str, company_id: str) -> None:
        ...
        if role == "employee":
            ...
            if lang == "en":
                ...
                text = f"The employee has signed \"{title}\" for {company_name}. Please add the employer signature in BauPass admin."
            else:
                subject = f"Vertrag vom Mitarbeiter unterschrieben — {title}"
                text = f"Der Mitarbeiter hat „{title}“ für {company_name} unterschrieben. Bitte Arbeitgeber-Signatur in BauPass ergänzen."
        else:
            recipient = str(form.get("employee_email") or "").strip()
            worker_id = contract.get("worker_id")
            if not recipient and worker_id:
            ...
            if lang == "en":
                ...
                text = f"The employer has signed \"{title}\". You can download the signed PDF from your BauPass app or sign link."
            else:
                subject = f"Vertrag vom Arbeitgeber unterschrieben — {title}"
                text = f"Der Arbeitgeber hat „{title}“ unterschrieben. Das signierte PDF steht in der BauPass-App bzw. über den Signatur-Link bereit."
