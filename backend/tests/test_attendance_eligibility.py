...
def test_company_plan_blocks_worker_without_assignment(db_conn):
    db_conn.execute(
        """
        INSERT INTO worker_deployment_days (company_id, worker_id, work_date, location_label)
        VALUES ('co-1', 'wrk-2', '2026-06-09', 'Berlin')
        """
    )
    db_conn.execute(
        "INSERT INTO workers (id, company_id, worker_type, first_name, last_name) VALUES ('wrk-2', 'co-1', 'worker', 'Anna', 'Test')"
    )
    db_conn.commit()
    worker = db_conn.execute("SELECT * FROM workers WHERE id = 'wrk-1'").fetchone()
    result = worker_may_auto_attend_today(
...
def test_allows_night_shift_window(db_conn):
    db_conn.execute(
        """
        INSERT INTO worker_deployment_days (company_id, worker_id, work_date, location_label, shift_start, shift_end)
    ...
    worker = db_conn.execute("SELECT * FROM workers WHERE id = 'wrk-1'").fetchone()
    result = worker_may_auto_attend_today(
        db_conn, worker, target_date=date(2026, 6, 9), now=__import__("datetime").datetime(2026, 6, 9, 23, 0)
    )
    assert result["ok"] is True
    assert result["dayType"] == "scheduled"
