...
SHOW_WAITRESS_QUEUE_WARNINGS = _env_flag("BAUPASS_WAITRESS_QUEUE_WARNINGS")
...
def _default_mode() -> str:
    explicit = str(os.getenv("BAUPASS_SERVER_MODE", "")).strip().lower()
    if explicit in {"dev", "prod"}:
        return explicit
...
def _prepare_runtime(mode: str) -> None:
    ...
    run_dunning_on_boot = _env_flag("BAUPASS_RUN_DUNNING_ON_BOOT")
    run_backup_on_boot = _env_flag("BAUPASS_BACKUP_ON_BOOT")

    if _env_flag("BAUPASS_SEED_DEMO_ENTERPRISE"):
        try:
    ...
    if suspended:
        print(f"[baupass] Auto-suspended {len(suspended)} company/ies due to overdue invoices", flush=True)
    ...
    archive_on_boot = _env_flag("BAUPASS_ARCHIVE_ACCESS_LOGS_ON_BOOT")
    if archive_on_boot:
        try:
            with app.app_context():
    ...
    try:
        ...
        if postgres_runtime_enabled() and _env_flag("BAUPASS_PG_DR_SNAPSHOT_ON_BOOT"):
...
def _serve_dev() -> None:
    print(f"[baupass] Dev server: host={HOST}, port={PORT}", flush=True)
    app.run(host=HOST, port=PORT, debug=True, use_reloader=True)


def _serve_prod() -> None:
...
def main(mode: str = "auto") -> None:
    ...
    if selected_mode not in {"dev", "prod"}:
        raise ValueError(f"Unsupported mode: {mode}")

    _prepare_runtime(selected_mode)
