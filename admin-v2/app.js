...
import {
  PAYSLIP_ASSET_V,
  PAYSLIP_LIST_PAGE_SIZE,
  applyPayslipFilterPrefs,
  dismissPayslipKeysHint,
  downloadPayslipAuditCsv,
  isPayslipKeysHintDismissed,
  openPayslipConfirmModal,
  paginatePayslipBatches,
  payslipEmptyStateHtml,
  payslipMatchHint,
  trackPayslipMetric,
  writePayslipFilterPrefs,
} from "./payslip-studio-ux.js?v=20260820lohnStudio14";

void PAYSLIP_ASSET_V;
...
const payslipStudioState = {
  ...
  capturePromise: null,
  syncPromise: null,
  listVisibleBatches: PAYSLIP_LIST_PAGE_SIZE,
};

applyPayslipFilterPrefs(payslipStudioState);
...
function unlockPayslipIframeChrome(iframe) {
  try {
    const doc = iframe?.contentDocument;
    if (!doc) return;
    const html = doc.documentElement;
...
async function pullPayslipsFromLohn() {
  ...
  try {
    ...
    rememberPayslipPullResult(res);
    trackPayslipMetric("pull", { created: Number(res.createdCount || 0), ok: true });
    ...
    trackPayslipMetric("pull_error", { message: e?.message || "error" });
    showActionToast(e?.message || t("lohn.pullPayslipsFailed") || "Abruf fehlgeschlagen", true);
  }
}

const PAYSLIP_PULL_CACHE_KEY = "workpass-payslip-last-pull";

function rememberPayslipPullResult(res) {
  const payload = {
    at: Date.now(),
    created: Number(res?.createdCount || 0),
    pending: Number(res?.pendingCount || 0),
    companyId: String(activeCompanyId() || ""),
  };
  try {
    sessionStorage.setItem(PAYSLIP_PULL_CACHE_KEY, JSON.stringify(payload));
  } catch {
    /* ignore */
  }
...
function readPayslipPullResult() {
  try {
    const raw = sessionStorage.getItem(PAYSLIP_PULL_CACHE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
...
function persistPayslipStudioPrefs() {
  writePayslipFilterPrefs(payslipStudioState);
}

function syncPayslipKeysHintBar() {
  const bar = $("payslipKeysHintBar");
  if (!bar) return;
  bar.classList.toggle("hidden", isPayslipKeysHintDismissed());
}

function updatePayslipSyncChrome() {
  const el = $("payslipStudioSyncLine");
  if (!el) return;
  const last = readPayslipPullResult();
  const cid = String(activeCompanyId() || "");
  if (!last || (last.companyId && cid && last.companyId !== cid)) {
    el.textContent = t("lohn.syncNever") || "Noch kein Abruf in dieser Sitzung — „Von Lohn holen“.";
    el.classList.add("is-idle");
    el.classList.remove("is-fresh", "is-stale");
    return;
  }
  ...
  el.textContent =
    t("lohn.syncStatusLine", {
      when,
      created: String(last.created || 0),
      pending: String(last.pending || 0),
    })
    || `Letzter Abruf ${when} · ${last.created || 0} neu · ${last.pending || 0} offen`;
  el.classList.toggle("is-fresh", mins <= 15);
  el.classList.toggle("is-stale", mins > 15);
...
function payslipDocKindLabel(kind) {
  ...
  return t("lohn.docPayslip") || "Lohnabrechnung";
}

function pushPayslipSessionAudit(entry) {
  if (!Array.isArray(payslipStudioState.sessionAudit)) payslipStudioState.sessionAudit = [];
  payslipStudioState.sessionAudit.unshift({
    at: Date.now(),
    ...entry,
  });
  payslipStudioState.sessionAudit = payslipStudioState.sessionAudit.slice(0, 40);
...
async function loadPayslipAuditRows() {
  const cid = activeCompanyId();
  const params = new URLSearchParams({ inbox: "archive" });
  if (cid) params.set("company_id", cid);
  const data = await apiSoft(`/api/payroll/statements/pending?${params.toString()}`, { batches: [] }, 6000);
  const rows = [];
  for (const b of data?.batches || []) {
    for (const s of b.statements || []) {
      const status = String(s.status || "");
      if (status !== "released" && status !== "rejected") continue;
      rows.push({
        name: s.displayName || s.workerId || "—",
        status,
        period: s.documentPeriod || s.period || b.period || "",
        kind: normalizePayslipDocKind(s),
        at: s.releasedAt || s.reviewedAt || b.releasedAt || b.updatedAt || "",
        amount: formatPayslipMoney(s.netAmount ?? s.grossAmount, s.currency),
        source: "archive",
      });
    }
  }
  rows.sort((a, b) => String(b.at || "").localeCompare(String(a.at || "")));
  payslipStudioState.auditRows = rows.slice(0, 30);
}

function renderPayslipAuditPanel() {
  const host = $("payslipStudioAudit");
  if (!host) return;
  ...
  const session = Array.isArray(payslipStudioState.sessionAudit) ? payslipStudioState.sessionAudit : [];
  const archive = Array.isArray(payslipStudioState.auditRows) ? payslipStudioState.auditRows : [];
  const parts = [];
  parts.push(`<div class="payslip-audit-head-row">
    <div class="payslip-audit-head">${escapeHtml(t("lohn.auditTitle") || "Protokoll")}</div>
    <button type="button" class="ghost payslip-audit-export" data-payslip-ui="audit-export">${escapeHtml(t("lohn.auditExportCsv") || "CSV")}</button>
  </div>`);
  if (session.length) {
    parts.push(`<div class="payslip-audit-sub">${escapeHtml(t("lohn.auditSession") || "Diese Sitzung")}</div>`);
    for (const row of session.slice(0, 12)) {
      const when = new Date(Number(row.at || Date.now())).toLocaleString();
      parts.push(
        `<div class="payslip-audit-row"><strong>${escapeHtml(row.action || "")}</strong> · ${escapeHtml(row.name || "—")}<span class="muted"> · ${escapeHtml(when)}</span></div>`,
      );
    }
  }
  parts.push(`<div class="payslip-audit-sub">${escapeHtml(t("lohn.auditArchive") || "Archiv (kürzlich)")}</div>`);
  if (!archive.length) {
    parts.push(`<div class="payslip-audit-row muted">${escapeHtml(t("lohn.auditEmpty") || "Noch keine gesendeten Einträge.")}</div>`);
  } else {
...
async function togglePayslipAuditPanel() {
  payslipStudioState.auditOpen = !payslipStudioState.auditOpen;
  $("payslipStudioAuditBtn")?.classList.toggle("is-active", payslipStudioState.auditOpen);
  if (payslipStudioState.auditOpen) {
    const host = $("payslipStudioAudit");
    if (host) {
      host.classList.remove("hidden");
      host.innerHTML = `<div class="payslip-audit-row muted">${escapeHtml(t("common.loading") || "…")}</div>`;
    }
    try {
      await loadPayslipAuditRows();
    } catch {
      payslipStudioState.auditRows = [];
    }
  }
  renderPayslipAuditPanel();
}

function computePayslipMonthStats() {
  const byPeriod = Object.create(null);
  let weak = 0;
  for (const b of payslipStudioState.batches || []) {
    for (const s of b.statements || []) {
      const period = String(s.documentPeriod || s.period || b.period || "—");
      if (!byPeriod[period]) byPeriod[period] = { total: 0, ready: 0 };
      byPeriod[period].total += 1;
      if (s.canRelease) byPeriod[period].ready += 1;
      const match = String(s.matchStatus || "matched");
      if (match === "unmatched" || match === "ambiguous" || wasPayslipRematched(b.id, s.statementId || s.id)) {
        weak += 1;
      }
    }
  }
  const periods = Object.keys(byPeriod).sort().reverse().slice(0, 3);
  return { byPeriod, periods, weak };
}
...
function filterPayslipListStatements(stmts) {
  ...
  const docTypeFilter = String(payslipStudioState.docTypeFilter || "all");
  return (stmts || []).filter((s) => {
    ...
    if (docTypeFilter !== "all" && normalizePayslipDocKind(s) !== docTypeFilter) return false;
    if (archived) {
      const status = String(s.status || "");
...
function renderPayslipInboxChrome() {
  ...
  if (inbox === "archive") {
    ...
    renderPayslipDocTypeFilters(batches);
    return;
  }
...
function renderPayslipDocTypeFilters(batches) {
  const host = $("payslipDocTypeFilters");
  if (!host) return;
  ...
  const cur = payslipStudioState.docTypeFilter || "all";
  const opts = [
  ...
  host.innerHTML = opts
    .map(([id, label]) => {
      const n = id === "all" ? "" : ` (${counts[id] || 0})`;
      return `<button type="button" class="${cur === id ? "is-active" : ""}" data-payslip-doc-filter="${id}">${escapeHtml(label)}${escapeHtml(n)}</button>`;
    })
    .join("");
}
...
function collectReleasablePayslipBatches() {
  if (payslipStudioState.inbox === "archive") return [];
  const out = [];
  for (const b of payslipStudioState.batches || []) {
    const n = Number(b.releasableCount || 0);
    if (n <= 0) continue;
    let amount = 0;
    let currency = "EUR";
    for (const s of b.statements || []) {
    ...
    out.push({ id: String(b.id || ""), count: n, amount, currency, period: b.period || "", company: b.companyName || b.companyId || "" });
  }
  return out;
}

function updatePayslipStudioChrome() {
  const progress = $("payslipStudioProgress");
  const releaseAll = $("payslipStudioReleaseAll");
  const flat = flattenPayslipStatements();
  ...
  const releasable = collectReleasablePayslipBatches();
  const readyN = releasable.reduce((n, b) => n + b.count, 0);
  if (progress) {
    if (!total) {
      progress.textContent =
        payslipStudioState.inbox === "archive"
          ? (t("lohn.payslipArchiveNone") || "Kein Archiv für diese Firma.")
          : (t("lohn.payslipNone") || "Keine offenen Lohnabrechnungen.");
    } else if (payslipStudioState.inbox === "archive") {
      progress.textContent = t("lohn.payslipProgressArchive", { n: String(total) }) || `${total} im Archiv`;
    } else {
      const base =
        t("lohn.payslipProgress", { current: String(pos || "—"), total: String(total) })
        || `${pos || "—"} / ${total}`;
      const readyHint =
        readyN > 0
          ? ` · ${t("lohn.payslipReadyCount", { n: String(readyN) }) || `${readyN} bereit`}`
          : "";
      progress.textContent = `${base}${readyHint}`;
    }
  }
  if (releaseAll) {
    const openInbox = payslipStudioState.inbox !== "archive";
    releaseAll.disabled = !openInbox || readyN <= 0 || payslipStudioState.releaseInFlight;
    releaseAll.textContent =
      readyN > 0
...
async function releaseAllReviewedPayslips() {
  if (payslipStudioState.inbox === "archive") return;
  const batches = collectReleasablePayslipBatches();
  const readyN = batches.reduce((n, b) => n + b.count, 0);
  if (!readyN) {
    showActionToast(t("lohn.noneReadyToSend") || "Keine geprüften Abrechnungen bereit", true);
    return;
  }
  ...
  const ok = await openPayslipConfirmModal({
    title: t("lohn.releaseReviewed") || "Alle geprüften senden",
    body:
    ...
    warnings: uniqueTrust.slice(0, 8),
    confirmLabel: t("lohn.releaseReviewed") || "Senden",
  ...
  try {
    showActionToast(t("lohn.sending") || "Wird gesendet…");
    for (const b of batches) {
      const res = await api(`/api/payroll/statements/${encodeURIComponent(b.id)}/release-reviewed`, {
        method: "POST",
        body: "{}",
      });
      released += Number(res?.released || 0);
    }
    trackPayslipMetric("release_all", { released: released || readyN, batches: batches.length });
    toast(
      t("lohn.releaseReviewedDone", { n: String(released || readyN) })
        || `${released || readyN} gesendet`,
      "ok",
    );
    await refreshPayslipStudio({ keepSelection: true });
    broadcastLohnInboxChanged();
  } catch (err) {
    trackPayslipMetric("release_all_error", { message: err?.message || "error" });
    showActionToast(humanizeUserError(err), true);
  } finally {
...
function renderPayslipStudioList() {
  ...
  if (!batches.length) {
    const hasRaw = Boolean(payslipStudioState.batches?.length);
    const emptyTitle = archived
      ...
      : hasRaw
        ...
        : (t("lohn.payslipNone") || "Keine offenen Lohnabrechnungen.");
    const emptyBody = archived
      ? (t("lohn.emptyArchiveHint") || "Filter zurücksetzen oder Periode wechseln.")
      : hasRaw
        ? (t("lohn.emptyFilterHint") || "Filter anpassen oder Suche leeren.")
        : (t("lohn.emptyOpenHint") || "Neue Abrechnungen mit „Von Lohn holen“ abrufen.");
    const actions = archived || hasRaw
      ? `<button type="button" class="ghost" data-payslip-ui="toggle-filters">${escapeHtml(t("lohn.filtersToggle") || "Filter")}</button>`
      : `<button type="button" class="primary" data-payslip-ui="pull">${escapeHtml(t("lohn.pullPayslipsShort") || "Von Lohn holen")}</button>`;
    host.innerHTML = payslipEmptyStateHtml({
      title: emptyTitle,
      body: emptyBody,
      actionsHtml: `<div class="payslip-empty-actions">${actions}</div>`,
      escapeHtml,
    });
    updatePayslipStudioChrome();
    return;
  }
  const page = paginatePayslipBatches(batches, payslipStudioState.listVisibleBatches);
  host.innerHTML =
    page.visible
      .map((batch) => {
      const bid = String(batch.id || "");
      const stmts = Array.isArray(batch.statements) ? batch.statements : [];
      const releasable = stmts.filter((s) => s?.canRelease).length || Number(batch.releasableCount || 0);
      // Prefer visible/filtered canRelease count so batch actions stay accurate.
      ...
      const showKindHeaders = Object.keys(byKind).length > 1;
      const items = kindOrder
        .filter((k) => byKind[k]?.length)
        .map((kind) => {
          const head = showKindHeaders
            ? `<div class="payslip-doctype-head">${escapeHtml(payslipDocKindLabel(kind))} · ${byKind[kind].length}</div>`
            : "";
          return `${head}${byKind[kind].map(renderStmt).join("")}`;
        })
        .join("");
    ...
    .join("")
    + (page.hasMore
      ? `<div class="payslip-list-more">
          <button type="button" class="ghost" data-payslip-ui="list-more">${escapeHtml(
            t("lohn.listShowMore", { n: String(page.remaining) }) || `Weitere ${page.remaining} anzeigen`,
          )}</button>
        </div>`
      : "");
  updatePayslipStudioChrome();
...
async function renderPayslipIdentity(stmt) {
  ...
  const lockNote = locked
    ...
    : "";
  const matchHint = payslipMatchHint(match, t);
  const matchHintHtml = matchHint
    ? `<p class="payslip-match-hint muted small">${escapeHtml(matchHint)}</p>`
    : "";
  const reviewedBadge =
    !locked && stmt.reviewed
      ? `<span class="payslip-match is-reviewed">${escapeHtml(t("lohn.reviewed") || "Geprüft")}</span>`
      : !locked
        ? `<span class="payslip-match is-unreviewed">${escapeHtml(t("lohn.unreviewed") || "Noch nicht geprüft")}</span>`
        : "";
  card.innerHTML = `
    ...
    ${matchHintHtml}${lockNote}${warnHtml}`;
        ...
        if (iframe) {
          ...
          iframe.onload = () => {
            try {
              ...
              setTimeout(schedulePayslipPreviewFit, 400);
              warmPayslipCaptureLibs();
...
async function openPayslipReviewStudio(opts = {}) {
  ...
  payslipStudioState.listVisibleBatches = PAYSLIP_LIST_PAGE_SIZE;
  syncPayslipListCollapseChrome();
  syncPayslipKeysHintBar();
  updatePayslipSyncChrome();
  warmPayslipCaptureLibs();
  schedulePayslipPreviewFit();
  trackPayslipMetric("studio_open");
...
function selectPrevPayslipStatement() {
  const flat = flattenPayslipStatements();
  if (!flat.length) return;
  const idx = flat.findIndex(
    (x) =>
  ...
  if (prev) return selectPayslipStatement(prev.batchId, prev.statementId);
}

function isPayslipStudioOpen() {
  return Boolean($("payslipReviewStudio") && !$("payslipReviewStudio").classList.contains("hidden"));
}

function payslipKeyboardTargetIsEditable(el) {
...
function clickPayslipStudioAction(action) {
  const btn = $("payslipReviewStudio")?.querySelector?.(`[data-payslip-action="${action}"]`);
...
async function handlePayslipStudioKeydown(ev) {
  if (!isPayslipStudioOpen()) return;
  if (payslipKeyboardTargetIsEditable(ev.target)) return;
  const key = String(ev.key || "");
  const lower = key.toLowerCase();

  if (key === "Escape") {
    if ($("payslipReviewStudio")?.classList.contains("is-sheet-focus")) {
      ev.preventDefault();
      $("payslipFocusExitBtn")?.click();
      return;
    }
    if (!$("payslipSheetOverlay")?.classList.contains("hidden")) return;
    ev.preventDefault();
  ...
  if (key === "ArrowUp" || lower === "k") {
    ev.preventDefault();
    await selectPrevPayslipStatement();
    return;
  }
  if (lower === "f") {
    ev.preventDefault();
    if (!clickPayslipStudioAction("focus-sheet")) clickPayslipStudioAction("open-window");
    return;
  }
  if (key === "+" || key === "=") {
    ev.preventDefault();
  ...
  if (key === "Enter" || lower === "s") {
    if (payslipStudioState.inbox === "archive") return;
    ev.preventDefault();
    clickPayslipStudioAction("release");
    return;
  }
  if (lower === "r") {
    if (payslipStudioState.inbox === "archive") return;
    ev.preventDefault();
    clickPayslipStudioAction("reject");
  }
}
...
function handlePayslipChromeAction(ui) {
  ...
  if (action === "list-more") {
    payslipStudioState.listVisibleBatches =
      (Number(payslipStudioState.listVisibleBatches) || PAYSLIP_LIST_PAGE_SIZE) + PAYSLIP_LIST_PAGE_SIZE;
    renderPayslipStudioList();
    trackPayslipMetric("list_more", { visible: payslipStudioState.listVisibleBatches });
    return;
  }
  if (action === "audit-export") {
    const rows = [
      ...(payslipStudioState.sessionAudit || []).map((r) => ({
        status: r.action,
        name: r.name,
        at: r.at,
        source: "session",
    ...
    if (!rows.length) {
      showActionToast(t("lohn.auditEmpty") || "Noch keine Einträge.", true);
      return;
    }
    downloadPayslipAuditCsv(rows);
    showActionToast(t("lohn.auditExportOk") || "Protokoll als CSV exportiert");
    return;
  }
  if (action === "close") {
    closePayslipReviewStudio();
...
async function handlePayslipStudioClick(ev) {
  ...
  const docFilter = ev.target?.closest?.("[data-payslip-doc-filter]");
  if (docFilter) {
    payslipStudioState.docTypeFilter = docFilter.getAttribute("data-payslip-doc-filter") || "all";
    payslipStudioState.listVisibleBatches = PAYSLIP_LIST_PAGE_SIZE;
    persistPayslipStudioPrefs();
    renderPayslipStudioList();
    return;
  ...
  if (batchAct) {
    ...
    if (!batchId) return;
    if (batchAct.disabled || batchAct.getAttribute("aria-disabled") === "true") {
      showActionToast(t("lohn.noneReadyToSend") || "Keine geprüften Abrechnungen bereit", true);
      return;
    }
    if (action === "release-reviewed") {
      const batch = (payslipStudioState.batches || []).find((b) => String(b.id) === String(batchId));
      ...
      const ok = await openPayslipConfirmModal({
        title: t("lohn.releaseReviewed") || "Senden",
        body: detailBody,
        warnings: [...new Set(trust)].slice(0, 8),
        confirmLabel: t("lohn.releaseReviewed") || "Senden",
        cancelLabel: t("common.cancel") || "Abbrechen",
      });
      if (!ok) {
      ...
      trackPayslipMetric("release_batch", { released: res?.released || 0, batchId });
      toast(res.message || `${res.released || 0} gesendet`, "ok");
      pushPayslipSessionAudit({
        action: t("lohn.releaseReviewed") || "Alle geprüften senden",
        name: `${res.released || n} · ${batch?.period || batchId}`,
      });
      await refreshPayslipStudio({ keepSelection: true });
      broadcastLohnInboxChanged();
      if (payslipStudioState.auditOpen) {
        loadPayslipAuditRows().then(() => renderPayslipAuditPanel()).catch(() => {});
      }
      return;
    }
    if (action === "reject") {
      const reason = window.prompt(t("lohn.rejectReason") || "Grund (optional)", "") || "";
      const okReject = await openPayslipConfirmModal({
        title: t("lohn.rejectBatch") || "Stapel ablehnen",
        body: t("lohn.confirmRejectBatch") || "Gesamten Stapel ablehnen?",
        confirmLabel: t("lohn.rejectBatch") || "Ablehnen",
        cancelLabel: t("common.cancel") || "Abbrechen",
        danger: true,
      });
      if (!okReject) return;
      ...
      trackPayslipMetric("reject_batch", { batchId });
  ...
  if (action === "release") {
    ...
    const okSend = await openPayslipConfirmModal({
      title: t("lohn.sendToWorker") || "An Mitarbeiter senden",
      body: t("lohn.confirmSendWorker") || "Diese Lohnabrechnung an die Mitarbeiter-App senden?",
      warnings: trust,
      confirmLabel: t("lohn.sendToWorker") || "Senden",
      cancelLabel: t("common.cancel") || "Abbrechen",
    });
    if (!okSend) {
      return;
    ...
    try {
      ...
      pushPayslipSessionAudit({
        action: t("lohn.statusReleased") || "Gesendet",
        name: stmtNow?.displayName || stmtNow?.workerId || statementId,
        period: stmtNow?.documentPeriod || stmtNow?.period || "",
      });
      const flatBefore = flattenPayslipStatements();
      const idxBefore = flatBefore.findIndex(
        (x) =>
          String(x.batchId) === String(batchId) &&
          String(x.statementId) === String(statementId),
      );
      const nextBefore = flatBefore[idxBefore + 1] || null;
      await refreshPayslipStudio({ keepSelection: false });
      broadcastLohnInboxChanged();
      if (payslipStudioState.autoAdvanceAfterSend !== false && nextBefore) {
        const stillThere = flattenPayslipStatements().some(
          (x) =>
            String(x.batchId) === String(nextBefore.batchId) &&
            String(x.statementId) === String(nextBefore.statementId),
        );
        if (stillThere) {
          await selectPayslipStatement(nextBefore.batchId, nextBefore.statementId);
        }
      }
      if (payslipStudioState.auditOpen) {
        loadPayslipAuditRows().then(() => renderPayslipAuditPanel()).catch(() => {});
  ...
  if (action === "reject") {
    ...
    pushPayslipSessionAudit({
      action: t("lohn.statusRejected") || "Abgelehnt",
      name: stmtNow?.displayName || stmtNow?.workerId || statementId,
      period: stmtNow?.documentPeriod || stmtNow?.period || "",
    });
...
function wireLohnDrawer() {
  ...
  $("payslipKeysHintDismiss")?.addEventListener("click", () => {
    dismissPayslipKeysHint();
    syncPayslipKeysHintBar();
  });
  payslipStudioState.listCollapsed = readPayslipListCollapsed();
  ...
  document.addEventListener("keydown", (ev) => {
    ...
    handlePayslipStudioKeydown(ev).catch(() => {});
  });
