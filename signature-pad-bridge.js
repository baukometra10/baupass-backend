...
(function initBaupassSignaturePadBridge(global) {
  ...
  async function probeProviders(force, options = {}) {
    const onlyIds = Array.isArray(options?.onlyIds)
      ? new Set(options.onlyIds.map((id) => String(id)))
      : null;
    const now = Date.now();
    if (!force && probeCache && (now - probeCacheAt) < PROBE_TTL_MS && !onlyIds) {
  ...
  async function captureSignature(options = {}) {
    ...
    await probeProviders(true, preferredId ? { onlyIds: [preferredId] } : undefined);
