...
const WORKER_BUILD_TAG = "20260707voice13";
...
function ensureWorkerVoiceCaptureInput() {
  ...
  input.className = "worker-chat-voice-capture-input hidden-control";
...
function bindWorkerVoiceCaptureInput() {
  const input = ensureWorkerVoiceCaptureInput();
  if (!input || input.dataset.changeBound === "1") {
    return;
  }
...
function syncWorkerVoiceCaptureOverlay(mode = "") {
  syncWorkerChatComposeRefs();
  bindWorkerVoiceCaptureInput();
  ...
  const useNativeOverlay = activeMode === "mic"
    && !workerVoiceRecording
    && !sendBtn.disabled
    && shouldUseWorkerNativeVoiceCapture()
    && !workerComposeHasPayload();

  let shell = sendBtn.closest(".worker-chat-send-shell");
  if (!shell && sendBtn.parentElement) {
    shell = document.createElement("div");
    shell.className = "worker-chat-send-shell";
    sendBtn.parentElement.insertBefore(shell, sendBtn);
  ...
  if (useNativeOverlay) {
    ...
    input.setAttribute("aria-label", t("chatVoiceMic"));
    const hint = elements.workerChatFileHint || document.getElementById("workerChatFileHint");
    if (hint) {
      hint.classList.remove("hidden");
      hint.textContent = t("chatVoiceNativeTap");
    }
    return true;
  }
  ...
  if (!chatCard) {
    ...
    chatCard.innerHTML = `
      ...
      <div class="worker-chat-compose">
        ...
        <input type="file" id="workerChatVoiceCaptureInput" class="worker-chat-voice-capture-input hidden-control" accept="audio/x-m4a,audio/mp4,audio/mpeg,audio/*" capture="microphone" tabindex="-1" aria-hidden="true" style="display:none;" />
  ...
  try {
    ...
    if (mode === "mic") {
      ...
      if (shouldUseWorkerNativeVoiceCapture()) {
        syncWorkerVoiceCaptureOverlay("mic");
        return;
      }
      if (!activeRecorder) {
        showWorkerNotice(t("chatVoiceNotSupported"));
...
let deploymentLastActionAt = 0;
let deploymentLastActionKey = "";
...
function deploymentActionRecentlyHandled(actionKey) {
  ...
  const now = Date.now();
  if (key && key === deploymentLastActionKey && now - deploymentLastActionAt < 700) {
    return true;
  }
  deploymentLastActionKey = key;
  deploymentLastActionAt = now;
  return false;
}

function applyDeploymentDayDeclinedLocally(iso, reason = "") {
  const key = String(iso || "").slice(0, 10);
  deploymentPlanCachedDays = deploymentPlanCachedDays.map((day) => {
    if (deploymentDayIso(day) !== key) {
      return day;
    }
    return {
      ...day,
  ...
  if (elements.deploymentPlanList && deploymentPlanCachedDays.length) {
    elements.deploymentPlanList.innerHTML = renderDeploymentPlanCalendar(deploymentPlanCachedDays);
  }
}

async function runDeploymentDayUndo(iso) {
  if (!iso) return;
  try {
...
async function confirmDeploymentDecline() {
  const iso = deploymentDeclinePendingDate;
  if (!iso || deploymentDeclineInFlight) {
    return;
  }
  ...
  applyDeploymentDayDeclinedLocally(iso, reason);
  closeDeploymentDeclineModal();
  closeDeploymentDayDetailModal();
...
function handleDeploymentPlanClick(event) {
  ...
  if (detailUndoBtn) {
    ...
    void runDeploymentDayUndo(iso);
    return;
  }

  if (target.id === "deploymentDeclineModal") {
    if (Date.now() >= deploymentModalGuardUntil) {
      closeDeploymentDeclineModal();
    }
    return;
  }
  if (target.id === "deploymentDayDetailModal") {
    if (Date.now() >= deploymentModalGuardUntil) {
      closeDeploymentDayDetailModal();
    }
    return;
  ...
  if (!target.closest("#deploymentPlanList")) {
    return;
  }
  if (!workerToken) {
    return;
  ...
  const dayCell = target.closest("[data-dep-date]");
  ...
  if (declineBtn) {
    ...
    if (!iso || deploymentActionRecentlyHandled(`decline:${iso}`)) {
      return;
    }
    openDeploymentDeclineModal(findDeploymentPlanDay(iso));
    return;
  ...
  if (undoBtn) {
    ...
    if (!iso || deploymentActionRecentlyHandled(`undo:${iso}`)) {
      return;
    }
    void runDeploymentDayUndo(iso);
    return;
