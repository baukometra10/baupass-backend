...
const WORKER_BUILD_TAG = "20260719callFix1";
...
function workerVoiceCaptureAvailable() {
  ...
  return Boolean(window.SUPPIXChatVoice?.canRecordVoice?.());
}
...
function beginWorkerVoiceStreamRequest(mode) {
  ...
  window.SUPPIXChatVoice?.ensureMediaDevices?.();
  if (!window.SUPPIXChatVoice?.micInputAvailable?.() || !window.SUPPIXChatVoice?.requestAudioStream) {
    return null;
  }
...
function updateWorkerVoiceHint(seconds) {
  ...
  hint.classList.remove("hidden");
  if (isWorkerTouchDevice() && seconds < 1) {
    hint.textContent = t("chatVoiceMobileRecord");
  } else {
...
function triggerWorkerChatPrimaryAction(event) {
  ...
  if (mode === "mic" && !workerVoiceRecording) {
    window.SUPPIXChatVoice?.ensureMediaDevices?.();
    workerDebug("[chat-voice] capabilities", {
      mic: window.SUPPIXChatVoice?.micInputAvailable?.(),
      mediaRecorder: window.SUPPIXChatVoice?.hasMediaRecorder?.(),
      webAudio: window.SUPPIXChatVoice?.hasWebAudioRecording?.(),
      canRecord: window.SUPPIXChatVoice?.canRecordVoice?.(),
...
function workerLocationCaptureLabels() {
  return {
    ...
    weakSignal: t("chatLocationWeakSignal") || "GPS-Signal schwach — bitte ans Fenster oder ins Freie",
    capturingProgress: t("chatLocationCapturingProgress") || "Verfeinern… ±{m} m",
    cancel: t("chatLocationCancel") || "Abbrechen",
    inaccurate: t("chatLocationInaccurate") || "GPS zu ungenau (±{m} m). Bitte kurz ins Freie — max. {max} m am Handy.",
...
function workerLocationShareLabels() {
  return Object.assign({}, workerLocationCaptureLabels(), {
    ...
    accuracyGood: t("chatLocationAccuracyGood") || "Genauigkeit ±{m} m",
    accuracyApprox: t("chatLocationAccuracyApprox") || "Standort ungefähr · ±{m} m",
    desktopSendable: t("chatLocationDesktopSendable") || "Standort sendbar · ±{m} m",
...
async function sendWorkerChatMessage(options = {}) {
  ...
  const uploadFile = file
    ? (window.SUPPIXChatVoice?.prepareChatUploadFile?.(file) || file)
    : null;
